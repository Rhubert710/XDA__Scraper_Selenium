...
# edit for chromedriver location
...
for page in range(1, pages):
    ...
    for section in sections:
        ...
        for article in articles:
            ...
            headline = str(article.find_element_by_tag_name(
                'h4').text).replace(',', '')

            excerpt = str(article.find_element_by_class_name(
                'the-excerpt').text).replace(',', '')
        ...
        url = f'https://www.xda-developers.com/page/{pagepylint+1}/'

driver.close()
df = pd.DataFrame(total, columns=['Headline',
                  'Excerpt', 'Author', 'Date_Posted'])
